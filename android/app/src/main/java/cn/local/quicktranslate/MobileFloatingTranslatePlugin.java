...
public class MobileFloatingTranslatePlugin extends Plugin {
    ...
    @PluginMethod
    public void showFloatingTranslateFromClipboard(PluginCall call) {
        if (!MobileFloatingTranslateSettings.canDrawOverlays(getContext())) {
            call.reject("需要先开启手机悬浮窗权限", "E_OVERLAY_PERMISSION_REQUIRED");
            return;
        }

        MobileFloatingTranslateOverlay.get(getContext()).showFromClipboardOrManual("手动打开");
        call.resolve(createState());
    }
    ...
    private JSObject createState() {
        ...
        state.put("canListenKeyEvents", false);
        ...
        state.put("shortcutKeyCode", 0);
        state.put("shortcutLabel", "");
    ...
    private void syncFloatingBubbleService() {
        MobileFloatingBubbleService.sync(getContext());
    }
}
