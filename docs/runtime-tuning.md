...
Scope: Spring Boot backend on the ExcelCC production host, with the backend
service running as `kick-backend.service` and runtime env loaded from
`/www/wwwroot/excelcc/kick-backend/.env.production`.
...
```bash
JAVA_OPTS="-Xms2g -Xmx4g -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/www/wwwroot/excelcc/kick-backend/heapdump.hprof -XX:+ExitOnOutOfMemoryError -Dfile.encoding=UTF-8 -Duser.timezone=Asia/Shanghai"
...
- `-Xmx4g` leaves headroom for MySQL, Redis, Nginx, OS page cache, build tools,
  and short-lived deployment work.
...
```text
java -jar /www/wwwroot/excelcc/kick-backend/forum-1.0.0.jar
```
...
```ini
[Service]
EnvironmentFile=/www/wwwroot/excelcc/kick-backend/.env.production
ExecStart=
ExecStart=/bin/sh -c 'exec java $JAVA_OPTS -jar /www/wwwroot/excelcc/kick-backend/forum-1.0.0.jar'
```
...
ENV_FILE=/www/wwwroot/excelcc/kick-backend/.env.production \
PORT=8081 \
bash scripts/deploy/runtime-tuning-check.sh
```
...
curl -fsS http://127.0.0.1:8081/api/public/home-overview >/dev/null
```
...
`/www/wwwroot/excelcc/kick-backend/.env.production` and restart:

```bash
sudo cp /www/wwwroot/excelcc/kick-backend/.env.production.bak /www/wwwroot/excelcc/kick-backend/.env.production
sudo systemctl restart kick-backend.service
