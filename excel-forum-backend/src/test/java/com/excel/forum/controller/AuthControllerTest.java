...
class AuthControllerTest {
    ...
    @BeforeEach
    void setUp() {
        AuthController controller = new AuthController(userService, passwordEncoder, jwtUtil, forumEventService);
    ...
    void registerRejectsShortPassword() throws Exception {
        mockMvc.perform(post("/api/auth/register")
                        ...
                        .content("""
                                ...
                                """))
                .andExpect(status().isBadRequest())
                .andExpect(content().json("\"密码长度至少6位\""));
    ...
    void registerTrimsUsernameAndEmailBeforeSave() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/register")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"username":"  tester  ","email":"  user@example.com  ","password":"123456"}
                                """))
