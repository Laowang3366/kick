...
public class DraftController {
    ...
    public ResponseEntity<?> publishDraft(@RequestAttribute Long userId, @PathVariable Long id) {
        try {
            Map<String, Object> result = postDraftService.publishDraft(userId, id);
            Post post = (Post) result.get("post");
            java.util.Map<String, Object> response = new java.util.HashMap<>();
            response.put("id", post.getId());
            response.put("message", post.getStatus() != null && post.getStatus() == -1 ? "草稿已发布，等待审核" : "草稿已发布");
            if (result.containsKey("expGained")) {
                response.put("expGained", result.get("expGained"));
            }
            if (result.containsKey("pointsRewards")) {
                response.put("pointsRewards", result.get("pointsRewards"));
            }
