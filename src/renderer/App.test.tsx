...
describe('App', () => {
  ...
  it('shows a new desktop version in the settings update card', async () => {
          ...
          releases: [
            {
              version: '0.1.34',
              platform: 'windows',
              fileName: 'Quick-Translate-0.1.34.exe',
    ...
    expect(screen.getByText('版本 0.1.34')).toBeInTheDocument();
  ...
  it('opens the desktop installer URL when the packaged updater is unavailable', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '设置' }));
    expect(await screen.findByText('版本 0.1.34')).toBeInTheDocument();
          ...
          releases: [
            {
              version: '0.1.34',
              platform: 'android',
              fileName: 'Quick-Translate-Android-0.1.34.apk',
